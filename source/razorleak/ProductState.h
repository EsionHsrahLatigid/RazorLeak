                                     ...
                                     int presetIndex)
{
    ...
    if (! stream.write (magic.data(), magic.size())
        ...
        return yup::Result::fail ("Failed to write RazorLeak state header");
    ...
    for (const auto& parameter : parameters)
    {
        ...
        if (! std::isfinite (value)
            ...
            return yup::Result::fail ("Failed to write RazorLeak parameter state");
    }
                                     ...
                                     int& presetIndex)
{
    ...
    if (stream.read (loadedMagic.data(), loadedMagic.size()) != static_cast<int> (loadedMagic.size())
        || loadedMagic != magic)
        return yup::Result::fail ("Invalid RazorLeak state header");

    if (stream.readInt() != version)
        return yup::Result::fail ("Unsupported RazorLeak state version");

    const auto loadedPreset = stream.readInt();
    if (! yup::isPositiveAndBelow (loadedPreset, numPresets))
        return yup::Result::fail ("Invalid RazorLeak preset index");

    const auto parameterCount = stream.readInt();
    const auto parameters = processor.getParameters();
    if (parameterCount != static_cast<int> (parameters.size()))
        return yup::Result::fail ("Invalid RazorLeak parameter count");
    ...
    for (int i = 0; i < parameterCount; ++i)
    {
        ...
        if (id.isEmpty() || parameter == nullptr || ! std::isfinite (value)
            ...
            return yup::Result::fail ("Invalid RazorLeak parameter state");

        for (const auto& loadedID : loadedIDs)
            if (loadedID == id)
                return yup::Result::fail ("Duplicate RazorLeak parameter state");
